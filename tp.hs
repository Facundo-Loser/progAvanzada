...
hacerJugada n (e, j) = if (elem n jugadas) then (e, j-n) else error "jugada no posible"
...
mejorJug :: Estado -> Int
mejorJug (j, i) | j == H = if mod i 2 == 0 then 3 else 1
	            | j == C = if mod i 2 == 0 then 3 else 1
...
juegosGanadores i | i == 1 = [0]
                  | i == 2 = [2]
                  | (mod i 7) == 0 = juegosGanadores(i - 5) ++ [i]
                  | (mod i 7) /= 0 = juegosGanadores((i - 1) - 2) ++ [i - 1]